...
pub struct TokenCache<E: CacheEntry> {
    main: Cache<String, E>,
    reverse: Option<Cache<String, Vec<ReverseMember>>>,
    ttl: Duration,
}

impl<E: CacheEntry> TokenCache<E> {
    pub fn new(ttl: Duration, capacity: u64, with_reverse: bool) -> Self {
        let reverse = with_reverse.then(|| build_reverse_cache(capacity));
    ...
    pub fn insert(&self, key: &str, entry: E) {
        ...
        if let Some(reverse) = &self.reverse
            && let Some(reverse_key) = entry.reverse_key()
        {
            reverse_add(reverse, reverse_key, &key, Instant::now() + self.ttl);
        }
    ...
    pub fn delete_by_reverse_key(&self, reverse_key: &str) -> u64 {
        ...
        let Some(members) = reverse.get(reverse_key) else {
            return 0;
        };
        let count = members.len() as u64;
        reverse.invalidate(reverse_key);
...
fn reverse_add(cache: &Cache<String, Vec<ReverseMember>>, key: &str, member_key: &str, expires_at: Instant) {
    cache
        .entry(key.to_string())
