...
use super::{FIELD_AUTHOR, FIELD_COMMENT, FIELD_ID, FIELD_NOTE, FIELD_SUMMARY, FIELD_TAG, FIELD_TITLE, FIELD_TS};
...
fn read_owner_name(guard: &agdb::DbAny, article: agdb::DbId) -> Result<String, agdb::DbError> {
...
fn read_latest_note(guard: &agdb::DbAny, latest_version_id: &str) -> Result<String, agdb::DbError> {
    if latest_version_id.is_empty() {
        return Ok(String::new());
    }
    let Some(version) =
        crate::repository::graph::resolve_node_id_sync(guard, ENTITY_TYPE_VERSION, latest_version_id)?
    else {
        return Ok(String::new());
    };
    Ok(
...
fn read_tag_names(guard: &agdb::DbAny, article: agdb::DbId) -> Result<Vec<String>, agdb::DbError> {
    let edges = guard.exec(
        QueryBuilder::search()
            .from(article)
            .where_()
...
fn read_comment_contents(
    guard: &agdb::DbAny,
    article: agdb::DbId,
) -> Result<Vec<String>, agdb::DbError> {
