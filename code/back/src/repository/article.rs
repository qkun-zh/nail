...
use crate::repository::schema::{
    ...
    KEY_CONTENT_HASH, KEY_ID, KEY_SUMMARY, KEY_TITLE, KEY_TYPE, TagRow, UserRow, VersionRow,
    alias_of,
};
...
impl std::fmt::Display for CreateArticleError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::AuthorNotFound => formatter.write_str("author not found"),
            Self::TitleAlreadyExists => formatter.write_str("title already exists"),
...
impl std::fmt::Display for UpdateArticleError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::NotFound => formatter.write_str("article not found"),
            Self::TitleAlreadyExists => formatter.write_str("title already exists"),
            Self::Db(error) => write!(formatter, "database query failed: {error}"),
        }
    }
...
pub async fn create_article(
    ...
    guard.transaction_mut(|transaction| {
        if resolve_node_id_in_txn(transaction, ENTITY_TYPE_USER, author_id)?.is_none() {
            return Err(CreateArticleError::AuthorNotFound);
        }
        if !find_by_index_in_txn(transaction, KEY_TITLE, title)?.is_empty() {
        ...
        if !find_by_index_in_txn(transaction, KEY_CONTENT_HASH, content_hash)?.is_empty() {
            return Err(CreateArticleError::ContentHashExists);
        }
        let mut seen_tags = std::collections::HashSet::new();
        let mut tag_ids: Vec<String> = Vec::with_capacity(tag_names.len());
        for name in tag_names {
            if seen_tags.insert(name) {
                let tag = get_or_create_tag_in_txn(transaction, name)?;
                tag_ids.push(tag.id);
            }
        }
...
pub async fn update_article(
    ...
    guard.transaction_mut(|transaction| {
        let Some(article) = resolve_node_id_in_txn(transaction, ENTITY_TYPE_ARTICLE, article_id)?
            ...
            return Err(UpdateArticleError::NotFound);
        };
        let title_conflict = find_by_index_in_txn(transaction, KEY_TITLE, title)?
            .into_iter()
        ...
        let mut seen_tags = std::collections::HashSet::new();
        let mut new_ids: Vec<agdb::DbId> = Vec::with_capacity(tag_names.len());
        for name in tag_names {
            if seen_tags.insert(name) {
                let tag = get_or_create_tag_in_txn(transaction, name)?;
...
fn enrich_articles(guard: &agdb::DbAny, ids: &[agdb::DbId]) -> Result<Vec<ArticleView>, DbError> {
    ...
    let node_set: std::collections::HashSet<agdb::DbId> = ids.iter().copied().collect();
    let owner_of: std::collections::HashMap<agdb::DbId, agdb::DbId> = owner_edges
        .elements
        .iter()
    ...
    for edge in &tag_edges.elements {
        ...
        tags_by_article
            .entry(edge.from)
            .or_default()
            .push(TagRef { id: id.clone(), name: name.clone() });
    }
