...
async fn update_user_name_rejects_a_taken_name() {
    ...
    let first = create_user(
        &state.database,
        &common::hash::hash("a@example.com".as_bytes()).expect("hash must succeed"),
    )
    .expect("first");
    let second = create_user(
        &state.database,
        &common::hash::hash("b@example.com".as_bytes()).expect("hash must succeed"),
    )
    .expect("second");
...
async fn update_user_email_applies_the_new_hash() {
    let (state, _) = build_state(&test_config(), 0).await.expect("state");
    let old_hash = common::hash::hash("alice@example.com".as_bytes()).expect("hash must succeed");
    let new_hash =
...
async fn update_user_email_rejects_a_mismatched_old_hash() {
    ...
    assert!(matches!(
        update_user_email(
            &state.database,
            &user_id,
            &common::hash::hash("someone-else@example.com".as_bytes()).expect("hash must succeed"),
            &common::hash::hash("alice-new@example.com".as_bytes()).expect("hash must succeed")
        ),
