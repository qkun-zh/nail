use common::request::ArticleSearchParams;
...
async fn search_filters_by_iso8601_time_range_and_renders_utc_times() {
    let context = TestCtx::new().await.expect("test context");
    let actor = member(&context, "alice@example.com");
    let now = common::time::now_ms().expect("now");
    let titles = ["Recent One", "Middle One", "Old One"];
    let offsets_ms = [2, 3, 4];
    for (title, offset_hours) in titles.iter().zip(offsets_ms.iter()) {
        let article_id = common::time::uuidv7_min_for_ms(now - offset_hours * 3_600_000);
        let version_id = common::time::uuidv7_max_for_ms(now - offset_hours * 3_600_000);
    ...
    let from = common::time::format_rfc3339_utc(now - 150 * 60_000).expect("from");
    let to = common::time::format_rfc3339_utc(now - 90 * 60_000).expect("to");
    let request = ArticleSearchParams {
    ...
    assert_eq!(page.items[0].title, "Recent One");
    assert!(
        page.items[0].time.ends_with('Z'),
...
async fn seed_article(
    ...
    let article_id = common::time::uuidv7_min_for_ms(version_ms);
    let version_id = common::time::uuidv7_max_for_ms(version_ms);
    crate::repository::article::create_article(
        &context.state.database,
        &crate::repository::article::ArticleDraft {
            article_id: article_id.clone(),
...
async fn search_combines_keyword_range_time_author_tag() {
    ...
    .await
    .expect("rust tag");
    assert_eq!(page.items.len() as u64, 2, "rust tag hits A and C");
    ...
    .await
    .expect("alice author");
    assert_eq!(page.items.len() as u64, 2, "alice authored A and C");
    ...
    let from = common::time::format_rfc3339_utc(now - 7 * 3_600_000).expect("from");
    let page = crate::logic::search::search_articles(
        &context.state,
        &admin(&context),
        &ArticleSearchParams {
    ...
    .await
    .expect("from only");
    assert_eq!(page.items.len() as u64, 2, "from-only keeps A and B");

    let to = common::time::format_rfc3339_utc(now - 60_000).expect("to");
    let page = crate::logic::search::search_articles(
        &context.state,
        &admin(&context),
        &ArticleSearchParams {
    ...
    let from6 = common::time::format_rfc3339_utc(now - 6 * 3_600_000).expect("from6");
    let page = crate::logic::search::search_articles(
        &context.state,
        &admin(&context),
        &ArticleSearchParams {
    ...
    .await
    .expect("quantum+from");
    assert_eq!(page.items.len() as u64, 2, "quantum within 6h is A and B");

    let to3 = common::time::format_rfc3339_utc(now - 3 * 3_600_000).expect("to3");
    let page = crate::logic::search::search_articles(
        &context.state,
        &admin(&context),
        &ArticleSearchParams {
    ...
    assert_eq!(page.items[0].title, "Lunar Cache");
}
...
async fn space_separated_keywords_match_any_field_or() {
    ...
    assert_eq!(page.items.len() as u64, 3, "space must OR the terms");
