...
async fn search_hides_a_soft_deleted_article() {
    ...
    crate::logic::article::delete_article(
        ...
        &actor,
        &article_id,
        Some(common::request::DeleteMode::Soft),
    )
    .await
    .expect("soft delete");
...
async fn search_removes_a_hard_deleted_article() {
    ...
    crate::logic::article::delete_article(
        ...
        &admin_id,
        &article_id,
        Some(common::request::DeleteMode::Hard),
    )
    .await
    .expect("hard delete");
...
async fn search_order_is_stable_across_repeated_queries() {
    ...
    let second =
        crate::logic::search::search_articles(&context.state, &actor, &params(Some("stable")))
            .await
            .expect("second query");
    let ids = |page: &common::response::ListPage<
        common::response::search::SearchArticleItem,
    >|
     -> Vec<String> {
        page.items
            .iter()
...
async fn search_time_range_is_inclusive_at_the_boundaries() {
    ...
    .await
    .expect("create");
    let from = common::time::format_rfc3339_utc(now).expect("from");
...
async fn search_excludes_articles_outside_the_time_range() {
    ...
    let old_from = common::time::format_rfc3339_utc(now - 3_600_000).expect("from");
    let old_to = common::time::format_rfc3339_utc(now - 1_800_000).expect("to");
    let request = ArticleSearchParams {
