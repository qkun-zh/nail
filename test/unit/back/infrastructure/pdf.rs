...
use crate::infrastructure::pdf::{
    PdfStreamGuard, content_hash_rel_path, valid_content_hash,
};

fn feed(guard: &mut PdfStreamGuard, chunks: &[&[u8]]) -> Result<(), crate::infrastructure::pdf::PdfGuardError> {
    for chunk in chunks {
        guard.update(chunk)?;
    }
    guard.finish()
...
fn pdf_smaller_than_minimum_is_rejected() {
    ...
    assert!(matches!(error, crate::infrastructure::pdf::PdfGuardError::TooSmall { .. }));
}
...
fn pdf_over_the_size_limit_is_rejected() {
    ...
    assert!(matches!(error, crate::infrastructure::pdf::PdfGuardError::TooLarge { .. }));
}
...
fn pdf_without_header_is_rejected() {
    ...
    assert!(matches!(error, crate::infrastructure::pdf::PdfGuardError::BadHeader));
}
...
fn pdf_with_unsupported_version_is_rejected() {
    ...
    assert!(matches!(error, crate::infrastructure::pdf::PdfGuardError::BadVersion));
}
...
fn pdf_without_footer_is_rejected() {
    ...
    assert!(matches!(error, crate::infrastructure::pdf::PdfGuardError::BadFooter));
}
...
fn content_hash_rel_path_rejects_an_invalid_hash() {
    ...
    assert_eq!(valid_content_hash("abcdef1234567890abcdef1234567890"), true);
    assert_eq!(valid_content_hash("ABCDEF1234567890abcdef1234567890"), false);
}
