...
use crate::logic::user::{delete_user, list_users, read_user_profile, update_user};
...
#[tokio::test]
async fn read_user_profile_self_returns_name_by_default_and_email_hash_on_request() {
    let context = TestCtx::new().await.expect("test context");
    let user_id = member_id(&context, "alice@example.com").await;

    let data = read_user_profile(&context.state, &user_id, &user_id, true, false)
        .await
        .expect("self read");
    assert_eq!(data["name"].as_str(), Some(user_id.replace('-', "").as_str()));
    assert!(data.get("email_hash").is_none());

    let data = read_user_profile(&context.state, &user_id, &user_id, true, true)
        .await
...
#[tokio::test]
async fn read_user_profile_self_surfaces_a_missing_user() {
    let context = TestCtx::new().await.expect("test context");
    let error = read_user_profile(&context.state, "missing", "missing", true, false)
        .await
        .unwrap_err();
...
#[tokio::test]
async fn read_user_profile_other_requires_user_read_permission() {
    let context = TestCtx::new().await.expect("test context");
    let member = member_id(&context, "alice@example.com").await;
    let target = member_id(&context, "bob@example.com").await;
    let error = read_user_profile(&context.state, &member, &target, true, false)
        .await
        .unwrap_err();
...
#[tokio::test]
async fn read_user_profile_other_returns_id_name_and_email_hash() {
    let context = TestCtx::new().await.expect("test context");
    let admin = admin_id(&context).await;
    let target = member_id(&context, "alice@example.com").await;

    let data = read_user_profile(&context.state, &admin, &target, true, true)
        .await
        .expect("admin read");
...
#[tokio::test]
async fn read_user_profile_other_reports_a_missing_user() {
    let context = TestCtx::new().await.expect("test context");
    let admin = admin_id(&context).await;
    let error = read_user_profile(&context.state, &admin, "missing", true, false)
        .await
        .unwrap_err();
...
async fn delete_user_transfer_removes_the_user() {
    ...
    context.state.caches.deregister.insert(
        &key,
        crate::repository::cache::DeregisterTokenEntry {
...
async fn delete_user_transfer_rejects_a_token_bound_to_another_account() {
    ...
    assert_eq!(
        error,
        LogicError::bad_request("deregister token does not match your account")
    );
}
...
async fn delete_user_transfer_rejects_an_unknown_token_for_a_live_user() {
    ...
    .await
    .unwrap_err();
    assert_eq!(error, LogicError::bad_request("invalid or expired deregister token"));
}
...
async fn list_users_requires_permission() {
    let context = TestCtx::new().await.expect("test context");
    let member = member_id(&context, "alice@example.com").await;
    let error = list_users(&context.state, &member, 1, 8).await.unwrap_err();
    assert_eq!(error, LogicError::forbidden("you are denied"));
}

#[tokio::test]
async fn list_users_returns_the_paginated_list() {
    let context = TestCtx::new().await.expect("test context");
    let admin = admin_id(&context).await;
    member_id(&context, "alice@example.com").await;
    let data = list_users(&context.state, &admin, 1, 8).await.expect("list");
