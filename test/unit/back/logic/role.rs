...
async fn create_role_rejects_duplicate() {
    ...
    create_role(&context.state, &admin_id, "editor").expect("create");
    let err = create_role(&context.state, &admin_id, "editor").unwrap_err();
...
async fn read_role_not_found() {
    let context = TestCtx::new().await.expect("context");
    let admin_id = admin(&context);
    let err = read_role(&context.state, &admin_id, "missing").unwrap_err();
...
async fn delete_role_rejects_required_role() {
    ...
    let role = read_roles(&context.state, &admin_id, 1, 100)
        ...
        .expect("admin role");
    let err = delete_role(&context.state, &admin_id, &role.id).unwrap_err();
    let msg = format!("{err}");
...
async fn read_roles_is_paginated() {
    let context = TestCtx::new().await.expect("context");
    let admin_id = admin(&context);
    create_role(&context.state, &admin_id, "role-a").expect("a");
    create_role(&context.state, &admin_id, "role-b").expect("b");
    let page = read_roles(&context.state, &admin_id, 1, 1).expect("page");
...
async fn role_crud_round_trip_by_id() {
    let context = TestCtx::new().await.expect("context");
    let admin_id = admin(&context);
    let (role_id, name) = create_role(&context.state, &admin_id, "editor").expect("create");
    assert_eq!(name, "editor");
    let view = read_role(&context.state, &admin_id, &role_id).expect("read");
    ...
    .expect("update");
    assert_eq!(view.id, role_id);
    let view = read_role(&context.state, &admin_id, &role_id).expect("read after update");
    assert!(view.permissions.contains(&"Article::Read".to_string()));
    let view = delete_role(&context.state, &admin_id, &role_id).expect("delete");
    assert_eq!(view.id, role_id);
    let err = read_role(&context.state, &admin_id, &role_id).unwrap_err();
    assert_eq!(err, LogicError::not_found("role not found"));
}
