...
fn empty_request(limit: u64) -> SearchRequest {
    SearchRequest {
        query: None,
        ranges: vec![common::search::SearchRange::Title],
        from_seconds: None,
        to_seconds: None,
        offset: 0,
        limit,
...
fn query_request(query: &str, ranges: Vec<common::search::SearchRange>) -> SearchRequest {
    SearchRequest {
...
async fn sync_and_read_round_trips_an_article() {
    ...
    let outcome = index
        .read(
            &state.database,
            query_request("unique", vec![common::search::SearchRange::Title]),
        )
        .await
        .expect("read");
...
async fn sync_all_and_sync_user_skip_soft_deleted_articles() {
    ...
    let rebuilt = version_articles(
        &index
            .read(
                &state.database,
                query_request("soft", vec![common::search::SearchRange::Title]),
            )
            .await
            .expect("read"),
    )
...
async fn sync_user_refreshes_the_author_name() {
    ...
    let outcome = index
        .read(
            &state.database,
            query_request("article", vec![common::search::SearchRange::Title]),
        )
        .await
        .expect("read");
...
async fn sync_user_refreshes_the_author_name_of_their_comments() {
    ...
    let commenter_id = crate::repository::user::create_user(
        &state.database,
        &common::hash::hash("bob@example.com".as_bytes()).expect("hash must succeed"),
    )
    .expect("user");
    ...
    let outcome = index
        .read(
            ...
            SearchRequest {
                ...
                ranges: vec![common::search::SearchRange::Comment],
                ..empty_request(10)
            },
        )
        .await
...
async fn sync_drops_all_docs_of_a_soft_deleted_article() {
    ...
    let outcome = index
        .read(
            &state.database,
            query_request("soft article", vec![common::search::SearchRange::Title]),
        )
        .await
        .expect("read");
    let versions = version_articles(&outcome);
